...
from .distance import wdist_l2
from .grid import distance_map
from .utils import filter_scan


# change to class to define __get_item__()
...
class AbstractSom(eqx.Module):
    ...
    w_bu: Float[Array, "x y D"] = eqx.field(init=False)  # same as "prototype weights"
    ...
    def __init__(
        ...
        self.w_bu = jax.random.uniform(
            key, (self.shape + (self.in_size,)), dtype=jnp.float32
        )
    ...
    def __call__(self, input: InputData):
        ...
        dist = f_dist(self.w_bu, input_bu_v)
        ...
        new_self = self.bulk_set(
            {
                "w_bu": self.algo.f_update(lr, nbh, input_bu_v, self.w_bu),
